...
项目管理不仅包括代码的编写，还包括项目的计划、组织、协作和跟踪等方面。发者项目管理能力最重要的在于管理这一方面，而管理能力要体现在对于团队或者项目里的参与者的交流上，以及管理者所管理项目的活力上面。
...
首先，通过对这四个影响因素进行分析，可以得到每个影响因素不同数量对应的价值。随后，统计出开发者的四个行为的数量，来计算各影响因素的分数后,再乘以相应权重。最后，把这四个分数相加得到开发者的最后总分，即为项目管理能力的分数，分数越高，表示开发者的项目管理能力越高。 

## 四、评估指标模型

### 1、指标计算模型的设计

![avatar](https://github.com/tmyisgod/images/blob/8416a5a9482d4bc31ea94a27851036da029ed5e0/picture/1.png)

以下是开发者项目管理能力的计算公式：

公式（1）：

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image004.gif)

 

公式（2）：![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image006.gif)

其中A表示开发者项目管理能力值，![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image008.gif)表示该影响因素的权重因子，![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image010.gif)表示开发者管理项目中亲自回复的issue的数量，例如某个开发者总计回复了多少个issue，根据issue的数量给予得分。![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image012.gif)表示以该影响因素为自变量的函数值，函数![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image014.gif)即为相应影响因素的实际映射方式。例如某个开发者总计回复了多少个issue，根据issue的数量给予得分。
...
![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image016.jpg)
...
SPSS软件平台提供高级统计分析、大量机器学习算法、文本分析,具备开源可扩展性,可与大数据的集成,并能够无缝部署到应用程序中。通过该软件平台可以更好地为该指标的四个影响因素分析出权重的分布

SPSSAU共输出一个表格，表格中包括指标变异性，冲突性指标的具体值，并且得到信息量值（指标变异性*指标冲突性），并得到最后的权重（即信息量的归一化值）。如表2所示：

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image018.jpg)
...
对单个影响因素例如合并pr的数量记为pr_merged,在指标计算模型中它的具体价值与对管理能力的贡献值以分段函数的方式确定，下面以影响因素pr_merged为例，给出映射函数![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image020.gif)的设计依据，并进行合理性分析。这里先给出对应上述四个影响因素的映射函数![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image021.gif)的表达式如公式所示:

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image023.gif)

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image025.gif)

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image027.gif)

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image029.gif)
...
![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image031.jpg)

**图1 不同库里的所作pr数量不同对应开发者数量表**

 

![IMG_256](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image033.jpg)

**图2 不同库里的所作pr数量在不同区间的分布图**

 

上图1的表中第一列是开发者们在单个库里合并PR的数量，第二列是这个数量的总和。第三列是这个总和所占的百分比，第五列是之前总和的累计百分比。图2所示，我们可以看到pr数量在0-9这个区间占绝大部分，pr数量在10-19这个区间占第二多，pr数量在20-39这个区间占第三多，pr数量在其他的区间占比就很少。所以划分区间的得分依据如下：

图1的这张表的意义在于找到开发者合并pr数量在哪些区间更有价值，哪些区间价值不高，根据这个图2这个合并pr数量区间的分布来给不同区间的分值。从而防止开发者为了刷分数而在许多库里都有合并pr但是数量只有1这个情况的出现。

根据图1的合并Pr这张表以及可以看出：

有80%的合并pr的数量为1-9个，所以开发者在单个库里合并pr数量在这个区间的价值较低，但是由于开发者依然做出了贡献所以给予1分。

10-19这个区间大约占总数的10%，并且每一个数量的数值大小都低于10000，所以给予3分。

20-39这个区间的数量已经超过总数了90%，而且这个区间占比5%，单个pr数量的人数也逐渐变小。所以将其定为第三档，给予7分。

40-59这个区间占总比的2%，定为第四档给予10分。

60-79这个区间占总比的1%，定为第五档给予15分。

80-99这个区间占总比的0.5%，但是由于合并pr的数量没有超过100，所以依然只给予20分。

100以上的区间，由于占比较小并且已经能够说明开发者在单个库里的工作量已经很高了，所以区间分段跨度较大，给予的得分也比较高。从而能够让普通管理者和优秀管理者得到区分。并且开发者在单个库里合并pr数量超过100的数量也比较少，所以区间定为100-249、250-499、499-1000、1000以上。分数增长也比100以内的高为40、80、100、150。最后通过这个评判标准确定开发者本身在不同的库里对于合并pr这一个影响因素的分数，最后将每一个库的分数相加得到这个影响因素的分数。
...
![7US8@GU1}KGTQNI9@EA[58M](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image035.jpg)
...
## 六、GitHub开发者项目管理能力分析与可视化

### 1、  开发者项目管理能力最终的结果

通过在click house数据库里的GitHub全与数据进行分析及预处理后，通过上述的评估指标模型的计算公式，最终得到每个开发者的项目管理能力的得分。

下图为部分开发者的项目管理能力的得分：![IMG_256](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image037.jpg)

如图所示，第一列为开发者的用户名，第二列为开发者项目管理能力的最终得分。

### 2、开发者项目管理能力可视化

（1）开发者所创建所有项目的仓库名称，以及这些项目仓库的热度大小：

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image039.jpg)

（2）开发者在每个项目仓库中合并pr的数量：

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image041.jpg)

（3）开发者项目管理能力的四个影响因素的得分，以及排名。

![img](file:///C:/Users/tmy/AppData/Local/Temp/msohtmlclip1/01/clip_image043.jpg)
